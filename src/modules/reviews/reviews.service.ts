...
export class ReviewsService {
  ...
  return {
    ...
    async findOne (id:number){
        try {
            const review = await this.prisma.review.findUnique({where:{id},select:this.safeSelect})
    ...
    async createReview(data: CreateReviewDto , user : JwtUser) {
        console.log(user)
        try{
            const findUser = await this.prisma.user.findUnique({where:{id:user.userId}})
            if (!findUser) throw new NotFoundException("User not found")
            const review = await this.prisma.review.create({ data:{...data,userId:findUser.id,email:findUser.email}, select: this.safeSelect });
            return review;
        }catch(error){
            handlePrismaError(error);
        }
